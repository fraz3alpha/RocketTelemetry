...
// Based off code by arduino.cc user "Krodal".
//
// June 2012
//      first version
// July 2013 
...
typedef struct accel_t_gyro_reg {
        uint8_t x_accel_h;
        uint8_t x_accel_l;
        uint8_t y_accel_h;
        uint8_t y_accel_l;
...
typedef struct accel_t_gyro_value {
        sensor_values accel;
        int16_t temperature;
        sensor_values gyro;
        sensor_values mag;
...
typedef union accel_t_gyro_union
    {
      accel_t_gyro_reg reg;
      accel_t_gyro_value value;
    };
...
class MPU6050 {
  public:
    ...
    void enablePassthrough(boolean enable=true);
    void setGyroScale(uint8_t scale);
    void setAccelerometerScale(uint8_t scale);
    accel_t_gyro_value getData();
    sensor_values getAcceleration();
    sensor_values getGyro();
    sensor_values getMagnetometer();
    int16_t getRawTemperature();
    double getTemperature();
    void configureMagnetometer(uint8_t address=0x1E);
    
  private:
    int write(uint8_t address, int start, const uint8_t *pData, int size);
    int write_reg(uint8_t address, int reg, uint8_t data);
